...
var validatorCodes =
  (module.exports._validatorCodes = require('./validatorCodes'))

var ignoredValidationKeys = (module.exports._ignoredValidationKeys = [
  'msg',
  'description',
  'swaggerType',
])
...
module.exports._generateValidationChain = function (validationRules) {
  ...
  return _.reduce(
    validationRules,
    function (memo, validationRule) {
      _.each(
        _.difference(_.keys(validationRule), ignoredValidationKeys),
        function (validator) {
...
module.exports.getValidatorChain = function (
  keys,
  validationRules,
  validationModel,
  scope,
  ...
  if (
    ...
    req.route &&
    req.route.name &&
    _.has(validatorChainStore, req.route.name + value)
  ) {
...
module.exports.validateAttribute = function (
  descriptor,
  keys,
  scopeValue,
  validationRules,
  ...
  var validatorChain = this.getValidatorChain(
    keys,
    validationRules,
    validationModel,
    scope,
  ...
  var submittedValue =
    typeof scopeValue === 'function'
      ? scopeValue.apply(req)
...
function _createError(scope, descriptor, validator) {
  ...
  var message =
    validator.msg ||
    validatorCodes.messages[validator.name] ||
    validatorCodes.messages._default
...
module.exports.process = function (validationModel, req, options) {
  ...
  _.each(validationModel, function (validationRulesScope, scope) {
    ...
    if (!realScope) {
      addError(
        errors,
        _createError(scope, realScope, {
          msg: 'Invalid scope. Must be in [' + utils.getScopeKeys + '].',
        }),
        realScope,
        options
      )
...
module.exports.validateObject = function (
  descriptor,
  keys,
  scopeValue,
  validationRulesScope,
  ...
  try {
    ...
    _.each(validationRulesScope, function (validationRules, key) {
      var attribute = self.validateAttribute(
        describeKey(descriptor, key),
        keys.concat([key]),
        scopeValue,
        validationRules,
        validationModel,
    ...
    if (options.forbidUndefinedVariables === true) {
      keys.forEach(function (key) {
        ...
        if (invalid) {
          addError(
            errors,
            _createError(realScope, describeKey(descriptor, key), {
